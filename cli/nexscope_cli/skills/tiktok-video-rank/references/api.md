...
> The following 25 fields are returned in `data[*]` and correspond to the `columns` definitions. Although metric names start with `total`, they represent cumulative values **within the selected ranking period** (`columns` titles begin with "period…", meaning "period...").
...
| createdByAiText | string | Whether the video is AI-generated (`yes` / `no`, meaning yes / no) |
| salesFlagText | string | Whether the video is shoppable (`yes` / `no`, meaning yes / no) |
