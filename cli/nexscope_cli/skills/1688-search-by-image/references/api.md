...
| isOnePsale | string | Whether single-item dropshipping is supported (`yes` = yes / `no` = no) |
| isJxhy | string | Whether this is a selected sourcing product (`yes` = yes / `no` = no) |
| sellerIdentities | string | Seller identity (`Super Factory` = Super Factory / `Power Seller` = Power Seller / `TrustPass Member` = TrustPass member) |
| offerIdentities | string | Product tag (`Select` = Select) |
...
| hasPromotion | string | Whether a promotion is available (`yes` = yes / `no` = no) |
| promotionType | string | Promotion type |
| isPatentProduct | string | Whether this is a patented product (`yes` = yes / `no` = no) |
| isSelect | string | Cross-border Select product collection indicator |
| currency | string | Currency (fixed value "¥") |
| sourceType | string | Source type (fixed value "1688") |
| sourceTool | string | Source tool (fixed value "1688Image Search") |
