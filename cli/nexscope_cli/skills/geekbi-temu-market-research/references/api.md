...
curl -X POST "${NEXSCOPE_PROXY_BASE}/api/v1/tools/research/geekbi/temu/categorySearch" \
  ...
  -H "MODE_ID: ${MODE_ID}" \
  -H "APP_NAME: ${APP_NAME}" \
  -d '{"regionId":211,"keyword":"pets","totalSoldMin":1000,"sort":"totalSold","order":"desc","page":1,"size":3}'
```
...
The entry script echoes HTTP errors as JSON. Invalid `size: 0` was verified to return `{"error":"HTTP 400: Bad Request","details":"...size must be an integer[1,200]..."}` (size must be an integer in [1,200]), without printing a Python stack trace.
